...
$j.AnyPicker = $j.AnyPicker || {
	...
	defaults: // Defaults Settings
	{
		...
		headerTitle:
		{
			markup: "<span class='ap-header__title'>날짜선택</span>",
			type: "Text",
	...
	extra: // Common Temporary Variables
	{
		...
		bHasCSS3D : $j.CF.testProperties("perspective", ["WebkitPerspective", "MozPerspective", "OPerspective", "msPerspective", "perspective", "perspectiveProperty"], false),
		sCSSTransform: $j.CF.testProperties("transform", ["WebkitTransform", "MozTransform", "OTransform", "msTransform", "transform"], true),
		sCSSTransformStyle: $j.CF.setPropertiesStyle("transform", ["WebkitTransform", "MozTransform", "OTransform", "msTransform", "transform"]),
		sCSSTransition: $j.CF.testProperties("transition", ["WebkitTransition", "MozTransition", "OTransition", "msTransition", "transition"], true),
		bHasCSSAnimation: $j.CF.testProperties("animation", ["WebkitAnimation", "MozAnimation", "OAnimation", "msAnimation", "animation"], false),
						...
						if(sArrDataKeys.length > 0)
						{
							for(iKey in sArrDataKeys)
							{
								...
								if(sKey.search("plugin_AnyPicker_") !== -1)
								{
									...
									$j(".ap-overlay-" + sKey.replace("plugin_AnyPicker_", "")).remove(); // Remove AnyPicker DOM appended for a particular AnyPicker object
...
function AnyPicker(element, options)
{
	...
	apo.elem = element;
	var sLang = (options.lang !== undefined || options.lang !== null) ? options.lang : $j.CalenStyle.defaults.lang,
	io18n = $j.extend(true, {}, $j.AnyPicker.defaults.i18n, $j.AnyPicker.i18n[sLang], options.i18n),
	...
	if($j.CF.isValid(options.headerTitle))
		apo.tmp.headerTitleDefined = true;

	if($j.CF.compareStrings(apo.setting.theme, "Android"))
	{
		if(!$j.CF.isValid(options.rowHeight))
			apo.setting.rowHeight = 50;

		if(!$j.CF.isValid(options.visibleRows))
	...
	else if($j.CF.compareStrings(apo.setting.theme, "Windows"))
	{
		...
		if($j.CF.isValid(options.setButton))
			apo.tmp.bModified.set = true;
		if($j.CF.isValid(options.cancelButton))
			apo.tmp.bModified.cancel = true;
		if($j.CF.isValid(options.nowButton))
			apo.tmp.bModified.now = true;
		if($j.CF.isValid(options.clearButton))
	...
	if($j.CF.isValid(apo.setting.components) && apo.tmp.numOfComp === 0)
   	...
   	$j.AnyPicker.extra.sStartEv = ($j.AnyPicker.extra.bHasPointer ? ($j.AnyPicker.extra.bHasIE10Pointer ? 'MSPointerDown' : 'pointerdown') : ($j.AnyPicker.extra.bIsTouchDevice ? 'touchstart' : 'mousedown touchstart'));
    $j.AnyPicker.extra.sMoveEv = ($j.AnyPicker.extra.bHasPointer ? ($j.AnyPicker.extra.bHasIE10Pointer ? 'MSPointerMove' : 'pointermove') : ($j.AnyPicker.extra.bIsTouchDevice ? 'touchmove' : 'mousemove touchmove'));
    $j.AnyPicker.extra.sEndEv = ($j.AnyPicker.extra.bHasPointer ? ($j.AnyPicker.extra.bHasIE10Pointer ? 'MSPointerUp' : 'pointerup') : ($j.AnyPicker.extra.bIsTouchDevice ? 'touchend' : 'mouseup touchend'));
    $j.AnyPicker.extra.sLeaveEv = ($j.AnyPicker.extra.bHasPointer ? ($j.AnyPicker.extra.bHasIE10Pointer ? 'MSPointerLeave' : 'pointerleave') : ($j.AnyPicker.extra.bIsTouchDevice ? null : 'mouseleave'));
    $j.AnyPicker.extra.sCancelEv = ($j.AnyPicker.extra.bHasPointer ? ($j.AnyPicker.extra.bHasIE10Pointer ? 'MSPointerCancel' : 'pointercancel') : null);
    $j.AnyPicker.extra.sOutEv = ($j.AnyPicker.extra.bHasPointer ? ($j.AnyPicker.extra.bHasIE10Pointer ? 'MSPointerOut' : 'pointerout') : null);
...
AnyPicker.prototype = {
	...
	init: function()
	{
		...
		{
			apo.tmp.sInputElemTag = $j(apo.setting.inputElement).prop("tagName").toLowerCase();
		...
		if($j.CF.isValid(apo.setting.onInit))
	...
	_inputElementClicked: function(e)
	{
		...
		if(apo.tmp.sInputElemTag !== "" && !(apo.tmp.oInputElemValid.bIsListItem || apo.tmp.oInputElemValid.bIsSelect))
		{
			var $oInput = $j(apo.setting.inputElement), sElemValue;
	...
	_showPicker: function()
	{
		...
		var sTemp = "",
		bAddSectionHeader = (($j.CF.compareStrings(apo.setting.layout, "popup") || $j.CF.compareStrings(apo.setting.layout, "popover")) && apo.setting.viewSections.header.length > 0),
		bAddSectionFooter = (($j.CF.compareStrings(apo.setting.layout, "popup") || $j.CF.compareStrings(apo.setting.layout, "popover")) && apo.setting.viewSections.footer.length > 0),
		bAddSectionFooterFixedTop = ($j.CF.compareStrings(apo.setting.layout, "fixed") && $j.CF.compareStrings(apo.setting.vAlign, "bottom") && apo.setting.viewSections.footer.length > 0),
		bAddSectionFooterFixedBottom = ($j.CF.compareStrings(apo.setting.layout, "fixed") && $j.CF.compareStrings(apo.setting.vAlign, "top") && apo.setting.viewSections.footer.length > 0),
		bIsDateTimeField = ($j.CF.compareStrings(apo.setting.mode, "datetime") && apo.tmp.sDateTimeMode === "datetime"),
		...
		if($j.CF.compareStrings(apo.setting.layout, "popover"))
		{
			...
			iPopupHeight = $j.CF.compareStrings(apo.setting.rowsNavigation, "scroller") ? 300 : 350,
			iTooltipTop = $j(apo.setting.inputElement).offset().top - iPopupHeight,
			bTooltipTop = (iTooltipTop < iMinTop) ? false : true,
			iTooltipBottom = $j(apo.setting.inputElement).offset().top + $j(apo.setting.inputElement).height() + iPopupHeight,
		...
		if(bAddSectionHeader)
		{
			...
			if(bIsDateTimeField && $j.CF.compareStrings(apo.setting.theme, "Windows"))
			{
		...
		if($j.AnyPicker.extra.bHasCSSAnimation)
		{
			...
			if($j.CF.compareStrings(apo.setting.layout, "fixed"))
			{
				if($j.CF.compareStrings(apo.setting.vAlign, "top"))
					$j(apo.tmp.overlaySelector).find(".ap-cont").addClass("ap-top-slide-down");
				else if($j.CF.compareStrings(apo.setting.vAlign, "bottom"))
					$j(apo.tmp.overlaySelector).find(".ap-cont").addClass("ap-bottom-slide-up");
			}
			...
			setTimeout(function()
			{
				...
				if($j.CF.compareStrings(apo.setting.layout, "fixed"))
				{
					if($j.CF.compareStrings(apo.setting.vAlign, "top"))
					{
						$j(apo.tmp.overlaySelector).find(".ap-cont").removeClass("ap-top-slide-down");
						$j(apo.tmp.overlaySelector + " .ap-cont").css({"top": 0});
					}
					else if($j.CF.compareStrings(apo.setting.vAlign, "bottom"))
					{
						$j(apo.tmp.overlaySelector).find(".ap-cont").removeClass("ap-bottom-slide-up");
		...
		if($j.CF.compareStrings(apo.setting.mode, "datetime") && apo.tmp.sDateTimeMode === "datetime")
	...
	_hidePicker: function()
	{
		...
		if($j(apo.tmp.overlaySelector).length > 0)
		{
			...
			if($j.AnyPicker.extra.bHasCSSAnimation)
			{
				...
				{
					if($j.CF.compareStrings(apo.setting.vAlign, "top"))
						$j(apo.tmp.overlaySelector).find(".ap-cont").addClass("ap-top-slide-up");
					else if($j.CF.compareStrings(apo.setting.vAlign, "bottom"))
						$j(apo.tmp.overlaySelector).find(".ap-cont").addClass("ap-bottom-slide-down");
				}
				setTimeout(function()
				{
	...
	_adjustPicker: function(iNumOfComp)
	{
		...
		{
			iNumOfComp = $j.CF.isValid(iNumOfComp) ? parseInt(iNumOfComp) : apo.tmp.numOfComp;
			...
			if(!apo.setting.componentsCoverFullWidth)
			{
				...
				$j(apo.tmp.overlaySelector).find(".ap-content-middle").css({"padding-left": iPadding, "padding-right": iPadding});
			}
			...
			iContTop += 2;
			iContentTop = $j(apo.tmp.overlaySelector).find(".ap-header").height() - iContTop;
			$j(apo.tmp.overlaySelector).find(".ap-content-top").css({"top": iContentTop});
			iContTop -= 2;
			iContentBottom = $j(apo.tmp.overlaySelector).find(".ap-footer").height() + iContTop;
			$j(apo.tmp.overlaySelector).find(".ap-content-bottom").css({"bottom": iContentBottom});

			apo.tmp.iExt = Math.floor(apo.setting.visibleRows / 2);
		}

		var $oRelativeTo = $j((apo.setting.relativeTo === null) ? apo.setting.inputElement : apo.setting.relativeTo),
		...
		if($j.CF.compareStrings(apo.setting.layout, "popover"))
		{
			...
			iMinTop = iBodyScrollTop,
			iMaxTop = iBodyScrollTop + iWinHeight,
			iInputTop = $j(apo.setting.inputElement).offset().top - iBodyScrollTop,
			...
			iPopupWidth = $j(apo.tmp.overlaySelector).find(".ap-cont").outerWidth(),
			iPopupHeight = $j(apo.tmp.overlaySelector).find(".ap-cont").outerHeight(),
			...
			$j(apo.tmp.overlaySelector).find(".ap-cont").css({"position": "absolute", "left": iPopupLeft, "top": iPopupTop});
		}
		else if($j.CF.compareStrings(apo.setting.layout, "inline"))
		{
			$j(apo.tmp.overlaySelector).find(".ap-cont").css({"position": "absolute", "width": iWidth, "left": iLeft, "top": iTop});
		}
		else if($j.CF.compareStrings(apo.setting.layout, "relative"))
		{
			$oRelativeTo = $j((apo.setting.relativeTo === null) ? apo.setting.inputElement : apo.setting.relativeTo);
			var iDocLeft = 0,
			...
			$j(apo.tmp.overlaySelector).find(".ap-cont").css({"position": "absolute", "min-width": iWidth, "left": iLeft, "top": iTop});
		}
	...
	_addViewSectionComponents: function()
	{
		...
		for(iTempIndex1 = 0; iTempIndex1 < $j.AnyPicker.extra.sArrViewSections.length; iTempIndex1++)
		{
			...
			if(sArrComponents.length > 0)
			{
				for(iTempIndex2 = 0; iTempIndex2 < sArrComponents.length; iTempIndex2++)
				{
					...
					if(oComp !== undefined && oComp !== null && oComp !== "")
					{
						...
						{
							$j(apo.tmp.overlaySelector).find(".ap-header").append($oCompElem); // Append Component
							if($j.CF.compareStrings(oComp.type, "Button") && $j.CF.compareDataType(oComp.action, "function"))
						...
						{
							$j(apo.tmp.overlaySelector).find(".ap-content-top").append($oCompElem); // Append Component
							if($j.CF.compareStrings(oComp.type, "Button") && $j.CF.compareDataType(oComp.action, "function"))
								$oCompElem.on("click." + apo.setting.timestamp, {"apo": apo}, oComp.action); // Bind Action
						}
						else if($j.CF.compareStrings(sViewSection, "contentBottom"))
						{
							$j(apo.tmp.overlaySelector).find(".ap-content-bottom").append($oCompElem); // Append Component
							if($j.CF.compareStrings(oComp.type, "Button") && $j.CF.compareDataType(oComp.action, "function"))
								$oCompElem.on("click." + apo.setting.timestamp, {"apo": apo}, oComp.action); // Bind Action
						}
						else if($j.CF.compareStrings(sViewSection, "footer"))
						{
							var $oeButtonCont = $j(apo.tmp.overlaySelector).find(".ap-button-cont");
							if($oeButtonCont.length > 0)
								$oeButtonCont.append($oCompElem);
							else
								$j(apo.tmp.overlaySelector).find(".ap-footer").append($oCompElem); // Append Component
							if($j.CF.compareStrings(oComp.type, "Button") && $j.CF.compareDataType(oComp.action, "function"))
								$oCompElem.on("click." + apo.setting.timestamp, {"apo": apo}, oComp.action); // Bind Action
						}

						if($j.CF.compareStrings(sComp, "headerTitle") && $j.CF.compareStrings(oComp.contentBehaviour, "Static"))
							$j(apo.tmp.overlaySelector).find(".ap-header__title").text(apo.setting.i18n.headerTitle);
		...
		$j(apo.tmp.overlaySelector).find(".ap-button")
		...
		setTimeout(function()
		{
			$j(apo.tmp.overlaySelector).on("click." + apo.setting.timestamp, function(e)
			{
		...
		$j(apo.tmp.overlaySelector).find(".ap-cont, .ap-cont *").on("click." + apo.setting.timestamp, function(e)
		{
			e.stopPropagation();
		});

		$j(apo.tmp.overlaySelector).find(".ap-content-switch-date").click(function(e)
		{
			e.stopPropagation();
			apo._setDateTimeTabs("date");
		});

		$j(apo.tmp.overlaySelector).find(".ap-content-switch-time").click(function(e)
		{
			e.stopPropagation();
	...
	_setDateTimeTabs: function(sSelectedTab)
	{
		...
		$j(apo.tmp.overlaySelector).find(".ap-content-switch-tab").removeClass("ap-content-switch-selected");
		$j(apo.tmp.overlaySelector).find(".ap-content-switch-" + sSelectedTab).addClass("ap-content-switch-selected");
		var $oeDisabledTabs = $j(apo.tmp.overlaySelector).find("[data-type='" + sDisabledTab + "']"),
		$oeSelectedTabs = $j(apo.tmp.overlaySelector).find("[data-type='" + sSelectedTab + "']");
	...
	_setOutput: function()
	{
		...
		if(apo.tmp.oElemValid.bIsListItem || apo.tmp.oElemValid.bIsSelect)
		{
			...
			if(apo.tmp.oElemValid.bIsListItem)
				...
				sChild = "option";
			$j(apo.elem).find(sChild).each(function()
			{
				$oChildElem = $j(this);
...
AnyPicker.prototype = $j.extend(AnyPicker.prototype, {
	...
	reloadComponent: function(iComp, bSingleLoad)
	{
		...
		sAlign = apo.setting.rtl ? ($j.CF.compareStrings(oComp.textAlign, "left") ? "right" : $j.CF.compareStrings(oComp.textAlign, "right") ? "left" : "") : oComp.textAlign,
		...
		if($j.CF.compareStrings(apo.setting.rowsNavigation, "scroller+buttons") || $j.CF.compareStrings(apo.setting.rowsNavigation, "buttons"))
		...
		{
			bIsSelected = ($j.CF.isValid(oSData[iRowIndex].selected) ? oSData[iRowIndex].selected : false);
			...
			if($j.CF.compareStrings(apo.setting.theme, "Windows") && !$j.CF.isValid(apo.setting.rowView))
			{
				sTemp += "</div>";
		...
		if($oeComp.length > 0)
			...
			$j(apo.setting.parent).find(".ap-component-section").append(sTemp);
	...
	_bindEventsToComponent: function(iComp)
	{
		...
		if($j.CF.compareStrings(apo.setting.rowsNavigation, "scroller+buttons") || $j.CF.compareStrings(apo.setting.rowsNavigation, "scroller"))
		{
			$oCompCont.on($j.AnyPicker.extra.sStartEv + "." + apo.setting.timestamp, oData, apo._onStartDrag);
			$oCompCont.on($j.AnyPicker.extra.sMoveEv + "." + apo.setting.timestamp, oData, apo._onDrag);
			if($j.CF.isValid($j.AnyPicker.extra.sEndEv))
				$oCompCont.on($j.AnyPicker.extra.sEndEv + "." + apo.setting.timestamp, oData, apo._onEndDrag);
			if($j.CF.isValid($j.AnyPicker.extra.sOutEv))
				$oCompCont.on($j.AnyPicker.extra.sOutEv + "." + apo.setting.timestamp, oData, apo._onEndDrag);

			if($j.AnyPicker.extra.sMouseWheel.length > 0)
			{
				for(var iEventIndex = 0; iEventIndex < $j.AnyPicker.extra.sMouseWheel.length; iEventIndex++)
					$oCompCont.on($j.AnyPicker.extra.sMouseWheel[iEventIndex] + "." + apo.setting.timestamp, oData, apo._onMouseWheelScroll);
			}
		}
	...
	_onStartDrag: function(e)
	{
		...
		if($j.CF.compareStrings(apo.setting.theme, "Windows"))
		{
			var $oComp = $j(apo.tmp.overlaySelector).find("#ap-component-" + apo.tmp.iScrollingComp);

			if(apo.tmp.bIsManualDragging)
			{
				$j(apo.tmp.overlaySelector).find(".ap-component").removeClass("ap-component-modifying");
		...
		if(e.type === "mousedown")
		{
			$j(document).on("mousemove." + apo.setting.timestamp, e.data, apo._onDrag);
			$j(document).on("mouseup." + apo.setting.timestamp, e.data, apo._onEndDrag);
		}
	...
	_onDrag: function(e)
	{
		...
		if(apo.tmp.bIsTouched && (!$j.AnyPicker.extra.bHasPointer || $j.AnyPicker.extra.bHasPointer && (apo.tmp.bFromTouchStart || (!apo.tmp.bFromTouchStart && (iTSNew - apo.tmp.iTS > 1000)))))
		{
	...
	_setScrollingData: function(e)
	{
		...
		apo.tmp.oScrollingComp = $j("#ap-component-"+e.data.component).find(".ap-component-data");
		apo.tmp.iMinTopPos = 0;
		apo.tmp.iMaxTopPos = ($j(apo.tmp.oScrollingComp).find(".ap-row").length * apo.setting.rowHeight) - $j(apo.tmp.oScrollingComp).parent().innerHeight();
		apo.tmp.iTS = Date.now();
		apo.tmp.iCompDragStart = apo.tmp.iTS;
		apo.tmp.iOffsetStart = Math.abs($j(apo.tmp.oScrollingComp).position().top);
	...
	*/
	...
	_performTransition: function(iComp, sComp, iOffset, iTime, iMinTopPos, iMaxTopPos, iDir, iMinPosBounce, iMaxPosBounce, sSource, bStop)
	{
		...
		iTime = ($j.AnyPicker.extra.bHasPointer && apo.tmp.bIsTouched && sSource === "auto") ? 0 : iTime;

		if($j.AnyPicker.extra.bHasCSS3D)
		{
			oElem.style[$j.AnyPicker.extra.sCSSTransition] = $j.AnyPicker.extra.sCSSTransformStyle + " " + iTime + "s ease-out";
			oElem.style[$j.AnyPicker.extra.sCSSTransform] = "translate3d(0px, "+ -iOffset +"px, 0px)";
		}
		else
			$j(oElem).animate({"top": -iOffset}, 100);
	...
	_performTransitionAfter: function(oScrollingData, iMaxPosBounce, iMinPosBounce, sSource, iTime, bStop)
	{
		...
		setTimeout(function()
		{
			...
			if(bLTMin || bGTMax)
			{
				...
				{
					oElem.style[$j.AnyPicker.extra.sCSSTransition] = $j.AnyPicker.extra.sCSSTransformStyle + " " + iTime + "s ease-out";
					oElem.style[$j.AnyPicker.extra.sCSSTransform] = "translate3d(0px, "+ -Math.abs(iOffset) +"px, 0px)";
				}
				else
					$j(oElem).animate({"top": -Math.abs(iOffset)}, 100);
	...
	{
		var sMatrix = $j(oElem).hasClass("ap-component-data") ? $j(oElem).css("transform") : $j(oElem).find(".ap-component-data").css("transform"),
		iOffset = 0;
	...
	__scrollToSelectedRow: function()
	{
		...
		for(iTempIndex = 0; iTempIndex < apo.tmp.numOfComp; iTempIndex++)
		{
			var oArrSelected = $j(apo.tmp.overlaySelector).find("#ap-component-" + iTempIndex + " .ap-row-selected"),
	...
	setSelectedAndInvalidValuesForRows: function(bSetSelected)
	{
		...
		else if($j.CF.compareStrings(apo.setting.mode, "datetime"))
		{
			...
			$j(apo.tmp.overlaySelector).find(".ap-row").removeClass("ap-row-disabled ap-row-invalid ap-row-hidden");
	...
	*/
	_skipDisabledRow: function(sSource, iComp, iOffset, iDir)
	{
		...
		if($j.CF.isValid(oElem) && bIsRowDisabled)
		{
			...
			var iNewOffset = (iEnabledRow !== -1) ? ($j(oElem).find(sPartialId + iEnabledRow).position().top - (apo.tmp.iExt * apo.setting.rowHeight)) : -1;
	...
	_actionsOnComponentStop: function(sSource, iComp, iRow, iOffset, iDir)
	{
		...
		if(apo.tmp.sHeaderTitleType === "DynamicFunction")
		{
			...
			$j(apo.tmp.overlaySelector).find(".ap-header__title").text(sHeaderTitle);
		}
		else if(apo.tmp.sHeaderTitleType === "DynamicString")
		{
			...
			if(!apo.tmp.bIsManualDragging) // !$j.CF.compareStrings(apo.setting.theme, "iOS") &&
	...
	*/
	_findRows: function(iComp, iRow, iTotalRows, oElem, iDir)
	{
		...
		if(iDir === -1)
		{
			...
			{
				if(! $j(oElem).find("#ap-row-" + iComp + "-" + iTempIndex).hasClass("ap-row-disabled"))
					return iTempIndex;
			}
		}
		else if(iDir === 1)
	...
	*/
	...
	{
		var apo = this;
		var $oElemComp = $j(apo.tmp.overlaySelector).find("#ap-component-" + iComp),
		iElementIndex;
	...
	_getSelectedValueInComponent: function(iComp)
	{
		...
		{
			$oSelectedElem = $j(apo.tmp.overlaySelector).find("#ap-component-" + iComp + " .ap-row-selected");
		...
		else
		{
			for(iTempIndex = 0; iTempIndex < apo.tmp.numOfComp; iTempIndex++)
			{
				$oSelectedElem = $j(apo.tmp.overlaySelector).find("#ap-component-" + iTempIndex + " .ap-row-selected");
...
AnyPicker.prototype = $j.extend(AnyPicker.prototype, {
	...
	__setComponentsOfSelect: function()
	{
		...
		if(apo.setting.dataSource === null)
		{
			if(apo.tmp.oElemValid.bIsListItem || apo.tmp.oElemValid.bIsSelect)
			{
				...
				if($j.CF.compareStrings(apo.setting.headerTitle.contentBehaviour, "Dynamic") && $j.CF.isValid(apo.setting.headerTitle.format))
				{
	...
	__disableInvalidRowsOfSelect: function()
	{
		...
		for(iTempIndex1 = 0; iTempIndex1 < oData.length; iTempIndex1++)
		{
			...
			{
				$j(apo.elem).find("#ap-row-0-" + iTempIndex1).addClass("ap-row-disabled");
			}
		}
	}
...
AnyPicker.prototype = $j.extend(AnyPicker.prototype, {
	...
	*/
	__setComponentsOfDateTimePicker: function()
	{
		...
		if(!apo.tmp.headerTitleDefined && $j.CF.compareStrings(apo.setting.mode, "datetime") && !$j.CF.compareStrings(apo.setting.theme, "iOS"))
		{
          ...
          apo.setting.headerTitle.format = "yyyy-MM-dd (DDD)";
				else
					apo.setting.headerTitle.format = apo.setting.dateTimeFormat;
			}
		...
		if($j.CF.compareStrings(apo.setting.headerTitle.contentBehaviour, "Dynamic") && $j.CF.isValid(apo.setting.headerTitle.format))
		{
			...
			{
				apo.tmp.sHeaderTitleType = "DynamicString";
				apo.tmp.sArrHeaderTitleFormat = apo._matchRegex($j.AnyPicker.extra.sFormatRegex, apo.setting.headerTitle.format);
			}
		}
	},
	...
	__setDataSourceArrayForComponent: function(sCompName, sCompFormat)
	{
		...
		if(sCompName === "day")
		{
			if(sCompFormat === "DD")
			{
				for(iTempIndex = 0; iTempIndex < $j.AnyPicker.extra.en.veryShortDays.length; iTempIndex++)
				{
					oArrData.push(
					{
						val: iTempIndex,
			...
			{
				for(iTempIndex = 0; iTempIndex < $j.AnyPicker.extra.en.shortDays.length; iTempIndex++)
				{
					oArrData.push(
					{
						val: iTempIndex,
			...
			{
				for(iTempIndex = 0; iTempIndex < $j.AnyPicker.extra.en.fullDays.length; iTempIndex++)
				{
					oArrData.push(
					{
						val: iTempIndex,
	...
	{
		var apo = this;
		var sArrDateTimeFormat = apo._matchRegex($j.AnyPicker.extra.sFormatRegex, sDateTimeFormat);
	...
	_getRegexComponentForFormat: function(sInRegex, sFormat)
	{
		...
		else if($j.CF.compareStrings(sFormat, "y") || $j.CF.compareStrings(sFormat, "yyyy"))
		{
			sInRegex += "(\\d{1,4})";
		}
		...
		else if($j.CF.compareStrings(sFormat, "h") || $j.CF.compareStrings(sFormat, "hh"))
		{
			sInRegex += "(\\d{1,2})";
		}
		else if(sFormat === "m" || sFormat === "mm")
	...
	_parseInputDateTime: function(sDate)
	{
		...
		if(typeof sDate === "string")
		{
			...
			{
				if(($j.CF.compareStrings(oDate.me, "AM") || $j.CF.compareStrings(oDate.sm, "A")) && oDate.h === 12)
					oDate.H = 0;
				else if(($j.CF.compareStrings(oDate.me, "PM") || $j.CF.compareStrings(oDate.sm, "P")) && oDate.h < 12)
					oDate.H = oDate.h + 12;
				else
					oDate.H = oDate.h;
			}
	...
	_setInputDateTimeValue: function(sDateTimeFormat, sDateTimeValue, oThisDate)
	{
		...
		if(apo._isSupportedFormat(sDateTimeFormat))
		{
			...
			else if($j.CF.compareStrings(sDateTimeFormat, "y") || $j.CF.compareStrings(sDateTimeFormat, "yyyy"))
			{
				oThisDate.y = parseInt(sDateTimeValue);
			}
	...
	parseDisableValues: function()
	{
		...
		{
			// Parse Days
			if($j.CF.isValid(apo.setting.disableValues.day) && apo.setting.disableValues.day.length > 0)
			{
			...
			// Parse Dates
			if($j.CF.isValid(apo.setting.disableValues.date) && apo.setting.disableValues.date.length > 0)
			{
			...
			// Parse Times
			if($j.CF.isValid(apo.setting.disableValues.time) && apo.setting.disableValues.time.length > 0)
			{
				for(iTempIndex1 = 0; iTempIndex1 < apo.setting.disableValues.time.length; iTempIndex1++)
				{
					...
					bIsValidDay = $j.CF.isValid(oDayTime.day) && (oDayTime.day.length > 0),
					bIsValidVal = $j.CF.isValid(oDayTime.val) && (oDayTime.val.length > 0);
			...
			// Parse DateTimes
			if($j.CF.isValid(apo.setting.disableValues.datetime) && apo.setting.disableValues.datetime.length > 0)
			{
			...
			if(sCompName === "day")
			{
				for(iTempIndex1 = 0; iTempIndex1 < oData.length; iTempIndex1++)
				{
					for(iTempIndex2 = 0; iTempIndex2 < apo.tmp.oArrPDisable.day.length; iTempIndex2++)
					{
						...
						{
							$j(apo.elem).find("#ap-row-0-" + iTempIndex1).addClass("ap-row-disabled ap-row-invalid");
						}
					}
				}
			}
		...
		else
		{
			...
			for(iTempIndex1 = 0; iTempIndex1 < apo.setting.dataSource.length; iTempIndex1++) // change values of one component (dataSource)
			{
				...
				for(iTempIndex2 = 0; iTempIndex2 < oData1.data.length; iTempIndex2++) // dataSource.data values
				{
					...
					if(!bValidDate)
					{
						$j("#ap-component-" + iTempIndex1).find("#ap-row-" + iTempIndex1 + "-" + iTempIndex2).addClass("ap-row-disabled ap-row-invalid");
					}
				}
			}
		}
	...
	__getSelectedDate: function(bGetObject)
	{
		...
		{
			oComp = apo.setting.components[iTempIndex];
			oArrSelected = $j(apo.tmp.overlaySelector).find("#ap-component-" + iTempIndex + " .ap-row-selected");
			oElemSelected = (oArrSelected.length > 0) ? oArrSelected[0] : null;
	...
	*/
		...
		if($j.CF.isValid(apo.tmp.oArrPDisable))
		{
			...
			if(!bInvalidSelected && bHasDateComponent && apo.tmp.oArrPDisable.date.length > 0)
			{
				for(iTempIndex1 = 0; iTempIndex1 < apo.tmp.oArrPDisable.date.length; iTempIndex1++)
				{
					...
					else if($j.CF.isValid(oDateRecord.start) && $j.CF.isValid(oDateRecord.end))
					{
					...
					else if($j.CF.isValid(oDateRecord.min) && $j.CF.isValid(oDateRecord.max))
					{
				...
				for(iTempIndex1 = 0; iTempIndex1 < apo.tmp.oArrPDisable.time.length; iTempIndex1++)
				{
					...
					if(bIsValidDay)
					{
						for(iTempIndex2 = 0; iTempIndex2 < oDayRecord.val.length; iTempIndex2++)
						{
							...
							else if($j.CF.isValid(oTimeRecord.start) && $j.CF.isValid(oTimeRecord.end))
							{
							...
							else if($j.CF.isValid(oTimeRecord.min) && $j.CF.isValid(oTimeRecord.max))
							{
	...
	_findValidSelectedDate: function(bIsBefore, dDate, sType)
	{
		...
		{
			if($j.CF.isValid(apo.setting.minValue) && apo.compareDates(apo.setting.minValue, dDate) >= 0)
			{
				dDate = new Date(apo.setting.minValue);
				bIsBefore = false;
			}

			if($j.CF.isValid(apo.setting.maxValue) && apo.compareDates(apo.setting.maxValue, dDate) <= 0)
			{
			...
			do
			{
				...
				{
					if((!$j.CF.isValid(apo.setting.minValue) || ($j.CF.isValid(apo.setting.minValue) && apo.compareDates(apo.setting.minValue, dDate) <= 0)) && (!$j.CF.isValid(apo.setting.maxValue) || ($j.CF.isValid(apo.setting.maxValue) && apo.compareDates(apo.setting.maxValue, dDate) >= 0)))
					{
						return dDate;
					}
				}

			}while((!$j.CF.isValid(apo.setting.minValue) || ($j.CF.isValid(apo.setting.minValue) && apo.compareDates(apo.setting.minValue, dDate) <= 0)) && (!$j.CF.isValid(apo.setting.maxValue) || ($j.CF.isValid(apo.setting.maxValue) && apo.compareDates(apo.setting.maxValue, dDate) >= 0)));
		}
		else if(sType === "time")
		{
			if($j.CF.isValid(apo.setting.minValue) && apo.compareTimes(apo.setting.minValue, dDate) >= 0)
			{
				dDate = new Date(apo.setting.minValue);
				bIsBefore = false;
			}

			if($j.CF.isValid(apo.setting.maxValue) && apo.compareTimes(apo.setting.maxValue, dDate) <= 0)
			{
				dDate = new Date(apo.setting.maxValue);
				bIsBefore = true;
			}
			...
			{
				if(bIsBefore)
					dDate = new Date(dDate.getTime() - ($j.AnyPicker.extra.iMS.m * apo.setting.intervals.m));
				else
					dDate = new Date(dDate.getTime() + ($j.AnyPicker.extra.iMS.m * apo.setting.intervals.m));
				if(apo.__validateSelectedDate(dDate, true, false))
				{
					if((!$j.CF.isValid(apo.setting.minValue) || ($j.CF.isValid(apo.setting.minValue) && apo.compareTimes(apo.setting.minValue, dDate) <= 0)) && (!$j.CF.isValid(apo.setting.maxValue) || ($j.CF.isValid(apo.setting.maxValue) && apo.compareTimes(apo.setting.maxValue, dDate) >= 0)))
					{
						return dDate;
					}
				}
			}while((!$j.CF.isValid(apo.setting.minValue) || ($j.CF.isValid(apo.setting.minValue) && apo.compareTimes(apo.setting.minValue, dDate) <= 0)) && (!$j.CF.isValid(apo.setting.maxValue) || ($j.CF.isValid(apo.setting.maxValue) && apo.compareTimes(apo.setting.maxValue, dDate) >= 0)));
		}
		else if(sType === "datetime")
		{
			if($j.CF.isValid(apo.setting.minValue) && apo.compareDateTimes(apo.setting.minValue, dDate) >= 0)
			{
				dDate = new Date(apo.setting.minValue);
				bIsBefore = false;
			}

			if($j.CF.isValid(apo.setting.maxValue) && apo.compareDateTimes(apo.setting.maxValue, dDate) <= 0)
			{
				dDate = new Date(apo.setting.maxValue);
				bIsBefore = true;
			}
			...
			do
			{
				...
				if(apo.__validateSelectedDate(dDate, true, false))
				{
					if((!$j.CF.isValid(apo.setting.minValue) || ($j.CF.isValid(apo.setting.minValue) && apo.compareDateTimes(apo.setting.minValue, dDate) <= 0)) && (!$j.CF.isValid(apo.setting.maxValue) || ($j.CF.isValid(apo.setting.maxValue) && apo.compareDateTimes(apo.setting.maxValue, dDate) >= 0)))
					{
						return dDate;
					}
				}
			}while((!$j.CF.isValid(apo.setting.minValue) || ($j.CF.isValid(apo.setting.minValue) && apo.compareDateTimes(apo.setting.minValue, dDate) <= 0)) && (!$j.CF.isValid(apo.setting.maxValue) || ($j.CF.isValid(apo.setting.maxValue) && apo.compareDateTimes(apo.setting.maxValue, dDate) >= 0)));
		}
	...
	__setDaysOfMonthInDatePicker: function()
	{
		...
		if(iDateComp !== null)
		{
			...
			{
				var $oHiddenElem = $j("#ap-component-" + iDateComp).find("#ap-row-" + iDateComp + "-" + (iTempIndex - 1));
	...
	setDateInFormat: function(oInput, sType)
	{
		...
		else
		{
			oInput.iDate = {
				...
				M: (oInput.iDate.M !== undefined) ? oInput.iDate.M : $j.AnyPicker.extra.dToday.getMonth(),
				y: (oInput.iDate.y !== undefined) ? oInput.iDate.y : $j.AnyPicker.extra.dToday.getFullYear(),
	...
	{
		var apo = this;
		return new Date(dIpDate.getTime() - ((sIpTZOffset === undefined || sIpTZOffset === "" || sIpTZOffset === null) ?  -(dIpDate.getTimezoneOffset() * $j.AnyPicker.extra.iMS.m) : apo._getTZOffsetInMS(sIpTZOffset)));
	},
	...
	_getTZOffsetInMS: function(sTZOffset)
	{
		...
		if(sTZOffset === undefined || sTZOffset === "" || sTZOffset === null)
			iOffsetMS = -($j.AnyPicker.extra.dToday.getTimezoneOffset() * $j.AnyPicker.extra.iMS.m);
		else
		{
			var sArrTZOffset = apo._matchRegex(/^([+|-]{1})([0-1]{0,1}[0-9]{1}):([0-6]{0,1}[0-9]{1})$/, sTZOffset);
			iOffsetMS = parseInt(sArrTZOffset[2]) * $j.AnyPicker.extra.iMS.h + parseInt(sArrTZOffset[3]) * $j.AnyPicker.extra.iMS.m;
	...
	_getDifference: function(sUnit, dDate1, dDate2)
	{
		...
		if(sUnit === "ms")
			...
			iUnitDiff = (iMSDiff / $j.AnyPicker.extra.iMS.s);
		else if(sUnit === "m")
			iUnitDiff = (iMSDiff / $j.AnyPicker.extra.iMS.m);
		else if(sUnit === "h")
			iUnitDiff = (iMSDiff / $j.AnyPicker.extra.iMS.h);
		else if(sUnit === "d")
			iUnitDiff = (iMSDiff / $j.AnyPicker.extra.iMS.d);
		else if(sUnit === "M")
			iUnitDiff = (iMSDiff / $j.AnyPicker.extra.iMS.m);
		else if(sUnit === "y")
			iUnitDiff = (iMSDiff / $j.AnyPicker.extra.iMS.y);
